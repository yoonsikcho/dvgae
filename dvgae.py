...
class DVGAE(torch.nn.Module):
    ...
    def recon_loss(self, z1, z2, temp, pos_edge_index, neg_edge_index=None):
        ...
        """
        decode_p, prob_edge = self.decoder(z1, z2, temp, pos_edge_index, sigmoid=True, training=True)
        ...
        decode_n = self.decoder(z1, z2, temp, neg_edge_index, sigmoid=True, training=True, pos=False)
...
class InnerProductDecoder2(torch.nn.Module):
    ...
    def forward(self, z1, z2,  temp, edge_index, sigmoid=True, training=True, pos=True):
        ...
        if training: 
            if pos: 
                ...
                #value_network =  z2[edge_index[0],[0]]*z2[edge_index[0],[0]]  + z2[edge_index[1],[0]]*z2[edge_index[1],[0]] - sc*sc
...
N = int(data.x.size()[0])
if args.model == 'DVGAE':
    model = DVGAE(Encoder(data.x.size()[1], channels, data.train_pos_edge_index), Encoder2(data.x.size()[0], 2, data.train_pos_edge_index) , InnerProductDecoder2()).to(dev)
...
def train(epoch):
    ...
    loss,prob_edge = model.recon_loss(z1,z2, temp, train_pos_edge_index)

    loss = loss + (1.0 / data.num_nodes) * (model.kl_loss1()+model.kl_loss2())*beta
